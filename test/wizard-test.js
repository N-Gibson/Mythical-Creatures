...
describe('Wizard', function() {
  ...
  it('should only be able to cast 3 spells', function() {
    var wizard = new Wizard ('Bo');
    assert.equal(wizard.isRested, true);
    assert.equal(wizard.cast('MAKITH CRAFTS'));
    var wizard = new Wizard ('Bo');
    assert.equal(wizard.isRested, true);
    assert.equal(wizard.cast('MAKITH CRAFTS'));
    var wizard = new Wizard ('Bo');
    ...
    assert.equal(wizard.cast('I SHALL NOT MAKITH CRAFTS'));
    // create wizard
